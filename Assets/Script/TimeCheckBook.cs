...
public class TimeCheckBook : MonoBehaviour
{
    ...
    public void StopTimingAndSpawnObjects()
    {
        ...
        isTiming = false;
        int totalObjectCount = Mathf.FloorToInt(elapsedTime / spawnInterval);

        // Calculate the number of random objects based on elapsed time
        int randomObjectCount = Mathf.FloorToInt(elapsedTime / spawnInterval);

        // Calculate the count of interval-based spawns
        int intervalObjectCount = totalObjectCount - randomObjectCount;

        // Spawn objects based on intervals
        for (int i = 0; i < intervalObjectCount; i++)
        {
            SpawnObject();
            Debug.Log("물체생성");
        }

        // Determine the maximum height for stacking
